...
class FRCNN(object):
    _defaults = {
        ...
        #--------------------------------------------------------------------------#
        # "model_path"    : 'log_reg/best_epoch_weights.h5',
        "model_path"    : 'log_esb/best_epoch_weights.h5',
    ...
    def detect_image(self, image, crop = False, count = False):
        ...
        # rgb_image_shape = np.array(np.shape(rgb_image)[0:2])
        #---------------------------------------------------#
        #   计算输入到网络中进行运算的图片的高和宽
        #   保证短边是600的
        #---------------------------------------------------#
        input_shape = get_new_img_size(image_shape[0], image_shape[1])
        # rgb_image_shape = get_new_img_size(rgb_image_shape[0], rgb_image_shape[1])
        ...
        # rgb_image   = cvtColor(rgb_image)
        #---------------------------------------------------------#
        #   给原图像进行resize，resize到短边为600的大小上
        #---------------------------------------------------------#
        image_data  = resize_image(image, [input_shape[1], input_shape[0]])
        # rgb_image_data  = resize_image(rgb_image, [rgb_image_shape[1], rgb_image_shape[0]])
        #---------------------------------------------------------#
        #   添加上batch_size维度
        #---------------------------------------------------------#
        image_data  = np.expand_dims(preprocess_input(np.array(image_data, dtype='float32')), 0)
        # rgb_image_data  = np.expand_dims(preprocess_input(np.array(rgb_image_data, dtype='float32')), 0)
        ...
        for i, c in list(enumerate(top_label)):
            ...
            weighted_sum    += score * predicted_class_int
            ...
            for i in range(thickness):
                ...
                # rgb_draw.rectangle([left + i, top + i, right - i, bottom - i], outline=self.colors[c])
            draw.rectangle([tuple(text_origin), tuple(text_origin + label_size)], fill=self.colors[c])
            # rgb_draw.rectangle([tuple(text_origin), tuple(text_origin + label_size)], fill=self.colors[c])
            draw.text(text_origin, str(label,'UTF-8'), fill=(0, 0, 0), font=font)
            # rgb_draw.text(text_origin, str(label,'UTF-8'), fill=(0, 0, 0), font=font)
            # rgb_draw.text((450,10), str("filling_pre = {}".format(weighted_sum)), fill=(255,0,0), font=font)
             # , rgb_draw
        if score_sum < 1:
            score_sum = 1
        weighted_sum = weighted_sum / score_sum
        draw.text((450,10), str("filling_pre = {}".format(weighted_sum)), fill=(255,0,0), font=font)
        del draw
        with open('weighted_sum_results2.txt', 'a') as f:
                 f.write(f"{weighted_sum}\n")
