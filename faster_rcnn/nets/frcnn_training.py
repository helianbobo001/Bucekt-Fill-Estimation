...
# volume_classes = tf.constant(
#     [15, 20, 25, 30, 40, 45, 50, 55, 60, 65, 70, 75, 80, 0], dtype=tf.float32)
...
# def classifier_cls_loss():
#     def _classifier_cls_loss(y_true, y_pred):
#         class_weights = tf.constant([0, 15, 20, 25, 30, 35, 40, 45, 50, 55, 60, 65, 70, 75, 80], dtype=tf.float32)
#         weighted_y_true = tf.multiply(y_true, class_weights)
#         weighted_y_pred = tf.multiply(y_pred, class_weights)
#         print  (weighted_y_pred, weighted_y_true)
#         return 0.5*K.sqrt(tf.reduce_mean(tf.square(weighted_y_true - weighted_y_pred)))+ 5*K.mean(K.categorical_crossentropy(y_true, y_pred))
#     return _classifier_cls_loss
...
#         weighted_y_pred = tf.multiply(y_pred, class_weights)
#         # Compute residual
#         residual = weighted_y_true - weighted_y_pred
#         # Compute smooth L1 loss
#         loss = tf.where(tf.abs(residual) < 1, 0.5 * tf.square(residual), tf.abs(residual) - 0.5)
#         return K.mean(loss)
#     return _classifier_cls_loss


# def classifier_cls_loss(y_true, y_pred):
#     class_weights = tf.constant([20, 25, 30, 40, 45, 50, 55, 60, 65, 70], dtype=tf.float32)
#     weighted_y_true = tf.multiply(y_true, class_weights)
#     weighted_y_pred = tf.multiply(y_pred, class_weights)
#     _classifier_cls_loss = tf.reduce_mean(tf.square(weighted_y_true - weighted_y_pred))
#     return _classifier_cls_loss

# def classifier_cls_loss():
#     def _classifier_cls_loss(y_true, y_pred):
#         crossentonpy = K.backend.sparse_categorical_crossentropy(
#             y_true, y_pred)
#         V_pred = tf.reduce_sum(y_pred * volume_classes, axis=-1)
#         V_true = tf.reduce_sum(y_true * volume_classes, axis=-1)
#         mse = tf.reduce_mean(tf.square(V_pred - V_true))

#         return crossentonpy+mse

#     # def _classifier_reg_loss(y_true, y_pred):
...
def absolute_percentage_error(y_true, y_pred):
    ...
    weighted_y_true = tf.multiply(y_true, class_weights)
    weighted_y_pred = tf.multiply(y_pred, class_weights)
    error = tf.abs(weighted_y_true - weighted_y_pred)  # Absolute Error
    return tf.reduce_mean(error)  # Mean Absolute Error
