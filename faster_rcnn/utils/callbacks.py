...
class LossHistory(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        with open(os.path.join(self.log_dir, "epoch_val_loss.txt"), 'a') as f:
            ...
            f.write("\n")
        # with open(os.path.join(self.log_dir, "epoch_mae.txt"), 'a') as f:
        #     f.write(str(logs.get('mae')))
        self.loss_plot()
        with open(os.path.join(self.log_dir, "epoch_mae.txt"), 'a') as f:
            f.write(str(logs.get('mae')))
        with open(os.path.join(self.log_dir, "epoch_val_mae.txt"), 'a') as f:
            f.write(str(logs.get('val_mae')))
        self.mea_plot()
    ...
    def mea_plot(self):
        iters = range(len(self.mae))

        plt.figure()
        plt.plot(iters, self.mae, 'red', linewidth = 2, label='train mae')
        # plt.plot(iters, self.val_mae, 'coral', linewidth = 2, label='val mae')
        try:
            if len(self.mae) < 25:
                num = 5
            else:
            ...
            plt.plot(iters, scipy.signal.savgol_filter(self.mae, num, 3), 'green', linestyle = '--', linewidth = 2, label='smooth train mae')
        except:
            pass
